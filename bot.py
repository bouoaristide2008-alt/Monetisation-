...
# -------------------------
### MODIFICATION ###
# Ajout d'un verrou pour sérialiser les accès à SQLite et éviter la corruption en multi-threading
...
async def handler_service_choice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data in ("tiktok", "facebook"):
        # ### MODIFICATION ###
        # Utilisation de context.user_data (plus idiomatique pour ConversationHandler)
...
async def handler_whatsapp(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Ne pas vider context.user_data ici si l'on souhaite le conserver jusqu'à l'annulation,
    # mais puisque l'état de la conversation est déjà terminé (ConversationHandler.END),
    # c'est acceptable.
    return ConversationHandler.END
...
def paystack_webhook():
    ...
    # ... (le reste de la vérification du webhook est inchangé)
...
def build_telegram_app() -> Application:
    ...
    conv = ConversationHandler(
        ...
        states={
            SERVICE: [CallbackQueryHandler(handler_service_choice)],
...
if __name__ == "__main__":
    if TELEGRAM_TOKEN and EXTERNAL_URL:
        try:
            telegram_app.bot.set_webhook(url=f"{EXTERNAL_URL}/telegram-webhook/{TELEGRAM_TOKEN}")
